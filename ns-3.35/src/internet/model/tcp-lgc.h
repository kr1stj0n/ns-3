 ...
 * \ingroup tcp
 *
 * \brief An implementation of LGC. This model implements all of the
 * endpoint capabilities mentioned in the LGC paper.
 */

class TcpLgc : public TcpLinuxReno
{
public:
  /**
...
private:
  ...
  double m_LgcPhi;			//!< Parameter used to estimate the amount of network congestion
  double m_LgcAlpha;			//!< Estimation gain
  double m_LgcLogP;			//!< Use ECT(0) for ECN codepoint
  uint32_t m_LgcCoef;			//!< Use ECT(0) for ECN codepoint
  DataRate m_LgcMaxDataRate;		//!< Use ECT(0) for ECN codepoint
