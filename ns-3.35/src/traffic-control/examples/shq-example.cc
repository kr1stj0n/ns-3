...
NS_LOG_COMPONENT_DEFINE ("ShqExample");
...
{
  LogComponentEnable ("ShqQueueDisc", LOG_LEVEL_INFO);

  std::string shqLinkDataRate = "1.5Mbps";
  std::string shqLinkDelay = "20ms";
  ...
  // SHQ params
  NS_LOG_INFO ("Set SHQ params");
  Config::SetDefault ("ns3::ShqQueueDisc::MaxSize", StringValue ("100p"));
  Config::SetDefault ("ns3::ShqQueueDisc::MeanPktSize", UintegerValue (meanPktSize));
  Config::SetDefault ("ns3::ShqQueueDisc::Tinterval", TimeValue (Seconds (0.02)));
  Config::SetDefault ("ns3::ShqQueueDisc::Alpha", DoubleValue (0.25));
  Config::SetDefault ("ns3::ShqQueueDisc::MaxP", DoubleValue (1.0));
  Config::SetDefault ("ns3::ShqQueueDisc::LinkBandwidth", DataRateValue (DataRate ("100Mbps")));
  Config::SetDefault ("ns3::ShqQueueDisc::UseEcn", BooleanValue (true));
  ...
  p2p.SetQueue ("ns3::DropTailQueue");
  p2p.SetDeviceAttribute ("DataRate", StringValue (shqLinkDataRate));
  p2p.SetChannelAttribute ("Delay", StringValue (shqLinkDelay));
  devn2n3 = p2p.Install (n2n3);
  // only backbone link has SHQ queue disc
  queueDiscs = tchShq.Install (devn2n3);
  ...
  if (writeForPlot)
    {
      filePlotQueueDisc << pathOut << "/" << "shq-queue-disc.plotme";
      filePlotQueueDiscAvg << pathOut << "/" << "shq-queue-disc_avg.plotme";
  ...
  if (st.GetNDroppedPackets (ShqQueueDisc::FORCED_DROP) != 0)
    {
  ...
  if (printShqStats)
    {
      std::cout << "*** SHQ stats from Node 2 queue ***" << std::endl;
      std::cout << "\t " << st.GetNDroppedPackets (ShqQueueDisc::UNFORCED_MARK)
                << " marks due to prob mark" << std::endl;
      std::cout << "\t " << st.GetNDroppedPackets (ShqQueueDisc::FORCED_DROP)
